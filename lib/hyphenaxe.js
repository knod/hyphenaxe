...
// These are `root` and `axeFactory`
	...
	*/
			...
			for ( let key in userOptions ) {
				...
				if ( availableOptions[ key ] ) {
					...
					if ( val === null || val === undefined ) {
						// `null` is a way to allow "deletion" of previous values
		...
		*/
			var split 	 = [],
				// escape any regex incompatible characters
		...
		for (let key in options) {
			var val = userOptions[key]
			if ( val !== undefined && val !== null ) { options[key] = userOptions[key] }
		}
