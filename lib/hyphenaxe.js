	...
	*/
		...
		* Returns true if a number is a positive integer */
		...
		if ( typeof wordStr !== 'string' ) {
			throw new TypeError( "The first argument to hyperaxe needs to be a string. It's a" + typeof wordStr )
		}
		// --- maxNumChars ---
		if ( typeof maxNumChars !== 'number' ) {
			throw new TypeError(  "The second argument to hyperaxe is not a number (a positive integer greater than 0): " + maxNumChars  )
		} else if ( !isPositiveInt( maxNumChars ) || !(maxNumChars > 0) ) {
			throw new RangeError( "The second argument to hyperaxe is a number, but not a positive integer greater than 0: " + maxNumChars )
		}
		// --- userOptions ---
		if ( userOptions ) {
			...
			// ??: add options.debug to prevent this warning?
			if ( !hasValidOptions ) { console.warn( "The object handed in for your userOptions doesn't contain any properties that will be used by hyphenaxe."); }
